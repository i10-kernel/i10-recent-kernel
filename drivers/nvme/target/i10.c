...
static void i10_target_send_ddgst(struct ahash_request *hash,
		struct i10_target_cmd *cmd)
{
...
static void i10_target_recv_ddgst(struct ahash_request *hash,
		struct i10_target_cmd *cmd)
{
	struct scatterlist sg;
	struct kvec *iov;
...
static void i10_target_setup_c2h_data_pdu(struct i10_target_cmd *cmd)
{
	...
	if (queue->data_digest) {
		...
		i10_target_send_ddgst(queue->snd_hash, cmd);
	}
...
static void i10_target_execute_request(struct i10_target_cmd *cmd)
{
	if (unlikely(cmd->flags & I10_TARGET_F_INIT_FAILED))
		i10_target_queue_response(&cmd->req);
	else
...
static void i10_target_prep_recv_ddgst(struct i10_target_cmd *cmd)
{
	...
	i10_target_recv_ddgst(queue->rcv_hash, cmd);
	queue->offset = 0;
...
static int i10_target_try_recv_data(struct i10_target_queue *queue)
{
	...
	if (cmd->rbytes_done == cmd->req.transfer_len)
		i10_target_execute_request(cmd);
...
static int i10_target_add_port(struct nvmet_port *nport)
{
	...
	ret = sock_setsockopt(port->sock, SOL_SOCKET, SO_RCVBUFFORCE,
			optval, sizeof(opt));
	if (ret) {
	...
	ret = sock_setsockopt(port->sock, SOL_SOCKET, SO_SNDBUFFORCE,
			optval, sizeof(opt));
	if (ret) {
